    ...
    const ctx = gsap.context(() => {
      if (contentRef.current) {
        gsap.from(contentRef.current, {
          scale: 0.85, opacity: 0, filter: 'blur(12px)', ease: 'power2.out',
          scrollTrigger: { trigger: sectionRef.current, start: 'top 80%', end: 'top 40%', scrub: 0.8 },
        })
      }
      if (glowRef.current) {
  ...
  return (
    <section ref={sectionRef} className="section" style={{ position: 'relative', overflow: 'hidden', padding: 'var(--section-pad) 24px' }}>
      ...
      <div ref={contentRef} style={{
        ...
        position: 'relative', zIndex: 1, willChange: 'transform, opacity, filter',
      }}>
        <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: 800, letterSpacing: '-0.03em',
          lineHeight: 1.15, marginBottom: '14px', color: '#ffffff' }}>
          Ready to understand{' '}<span className="text-gradient">any codebase?</span>
        </h2>
