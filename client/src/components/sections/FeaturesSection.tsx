...
function TiltCard({ children, style = {}, highlighted = false }: TiltCardProps) {
...
const FEATURES: FeatureItem[] = [
  {
    icon: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/><path d="M10 6.5h4M6.5 10v4M17.5 10v4M10 17.5h4"/></svg>),
  ...
  {
    icon: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/><path d="M11 8v6M8 11h6"/></svg>),
  ...
  {
    icon: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="M12 6v6l4 2"/><path d="M2 12h2M20 12h2M12 2v2M12 20v2"/></svg>),
    title: 'Impact Analysis',
    desc: 'Select any function and predict what breaks — affected files, dependent APIs, at-risk modules, and AI-suggested migration steps.',
    tag: 'Core Feature',
    highlighted: true,
  },
]
    ...
    const ctx = gsap.context(() => {
      if (headingRef.current) {
        gsap.from(headingRef.current, {
          y: 60, opacity: 0, scale: 0.95, filter: 'blur(6px)', ease: 'power2.out',
          scrollTrigger: { trigger: headingRef.current, start: 'top 85%', end: 'top 50%', scrub: 0.8 },
        })
      }
      cardsRef.current.forEach((card) => {
        if (!card) return
        gsap.from(card, {
          y: 80, opacity: 0, scale: 0.92, ease: 'power2.out',
          scrollTrigger: { trigger: card, start: 'top 90%', end: 'top 60%', scrub: 0.6 },
        })
      })
    }, sectionRef)
    return () => ctx.revert()
      ...
      <div className="container">
        <div ref={headingRef} style={{ textAlign: 'center', marginBottom: '64px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '14px' }}>
            <span className="badge"><span className="badge-dot" />Features</span>
          </div>
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: 800, letterSpacing: '-0.03em',
            lineHeight: 1.15, marginBottom: '12px', color: '#ffffff' }}>
            Everything to{' '}<span className="text-gradient">master any codebase</span>
          </h2>
          <p style={{ fontSize: '15px', color: 'rgba(235,235,245,0.55)',
          ...
          {FEATURES.map((f, i) => (
            <div key={f.title} ref={el => { cardsRef.current[i] = el }}>
              <TiltCard highlighted={f.highlighted}
                ...
                <div className="feature-icon" style={{ color: 'var(--accent-light)', position: 'relative', zIndex: 1 }}>{f.icon}</div>
                <div style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '0.06em',
                  textTransform: 'uppercase', color: 'var(--accent-light)',
                  marginBottom: '6px', position: 'relative', zIndex: 1, opacity: 0.7 }}>{f.tag}</div>
